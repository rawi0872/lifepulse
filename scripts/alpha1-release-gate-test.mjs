...
assert(verifier.includes("#nextron-question") && verifier.includes("What am I falling behind on?") && verifier.includes("Context") && verifier.includes("NEXTRON Alpha 1.1 human-first hierarchy"), "Verifier covers stable Alpha 1.1 NEXTRON human-first landmarks");
assert(verifier.includes("assertNextronHumanHierarchy(page, \"post-setup NEXTRON Alpha 1.1 hierarchy\")") && !verifier.includes("Command channel"), "Verifier post-setup NEXTRON gate rejects obsolete command-center copy");
assert(verifier.includes("askNextronHumanPath") && verifier.includes("openMoreIntelligence") && verifier.includes("NEXTRON production response arrived before pending acknowledgement could be observed"), "Verifier proves NEXTRON ask behavior without timing-fragile pending requirements");
assert(verifier.includes("nonPendingNextronAssistantCount") && verifier.includes("waitForNextronAskTerminal") && verifier.includes("NEXTRON SEND STATE"), "Verifier waits for terminal non-pending NEXTRON turns and prints safe send-state diagnostics");
