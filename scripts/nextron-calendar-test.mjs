...
assert(coach.includes('| "CALENDAR_QUERY"') && routeBody.includes('parsedRequest.intent === "CALENDAR_QUERY"') && routeBody.indexOf('parsedRequest.intent === "CALENDAR_QUERY"') < routeBody.indexOf('isNextronProviderEligibleRequest'), "O Calendar intent routes before generic provider path");
...
assert(coachPage.includes('allow_knowledge, allow_drive, allow_calendar, allow_journal') && coachPage.includes('.select(PREFERENCE_COLUMNS)'), "T Coach permission UI loads and returns Calendar with existing permission DTO");
...
assert(calendar.includes('if (!isNextronContextAllowed(args.permissions, "calendar")) return { ok: false, reason: "PERMISSION_DENIED"') && calendar.includes('if (!row) return { ok: false, reason: "DISCONNECTED"') && calendar.includes('row.status === "revoked"') && calendar.includes('reason: "RECONNECT_REQUIRED"'), "X connected false, revoked, or permission false cannot read Calendar");
assert(askRoute.includes('const { permissions } = normalizeNextronPreferences') && askRoute.includes('runNextronCalendarReadOnly({ supabase, userId, permissions'), "Y prompt cannot override saved Calendar permission resolver");
...
assert(calendar.includes('classifyGoogleOAuthTokenResponse') && calendar.includes('error === "invalid_grant"') && calendar.includes('REFRESH_TOKEN_EXPIRED') && calendar.includes('REFRESH_TOKEN_REVOKED') && calendar.includes('status: "revoked", last_error_code: "RECONNECT_REQUIRED"'), "Z6 permanent Google refresh failures are safely classified and require reconnect");
assert(settings.includes('Reconnect Google Calendar') && settings.includes('Your Google authorization expired or was revoked. Reconnect Google Calendar to continue.'), "Z7 Calendar reconnect-required UX avoids raw OAuth errors");
