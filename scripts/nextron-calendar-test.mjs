...
assert(calendar.includes('TOKEN_DECRYPT_FAILED') && calendar.includes('TOKEN_REFRESH_FAILED') && calendar.includes('MCP_HTTP_403') && calendar.includes('MCP_PREVIEW_NOT_ENABLED') && calendar.includes('MCP_SCOPE_DENIED') && calendar.includes('MCP_PROTOCOL_ERROR') && calendar.includes('MCP_TOOL_ERROR'), "Z4 Calendar live failures use safe diagnostic classifications");
assert(calendar.includes('classifyMcpToolError') && !calendar.includes('console.log') && !calendar.includes('console.error'), "Z5 MCP tool-error text is classified without logging private payloads");
