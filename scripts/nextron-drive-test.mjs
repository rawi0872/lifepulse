...
assert(tools.includes("isMetadataOnlyDriveSnippet") && tools.includes("!groundedHybridResults.every(isMetadataOnlyDriveSnippet)"), "Metadata-only Drive retrieval must fall through to body-backed Knowledge item search.");
assert(tools.includes("matchesQueryToken") && tools.includes("groundedHybridResults"), "Knowledge semantic results must be lexically grounded before use for named document questions.");
assert(knowledgeHybrid.includes('"phrase"') && knowledgeHybrid.includes('"verification"') && knowledgeHybrid.includes('"document"'), "Generic phrase/document terms must not retrieve unrelated Knowledge for named security prompts.");
