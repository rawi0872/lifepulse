...
import { WEALTH_CURRENCIES } from "@lifepulse/domain";
  ...
  return (
    <Card className="mb-3 p-3.5 sm:p-4">
      <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-end">
        ...
        <div className="min-w-0 w-full sm:w-28">
          <SimpleSelect
            label="Currency (optional)"
            options={[{ value: "", label: "Unknown" }, ...WEALTH_CURRENCIES.map((c) => ({ value: c, label: c }))]}
      ...
      <p className="mt-2 text-[10px] leading-relaxed text-[var(--text-muted)]">Leave currency empty when unknown — the budget will say so instead of guessing.</p>
    </Card>
  );
}
