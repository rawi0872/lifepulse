...
export function FinanceKpiCard({ label, value, delta, variant, isLoading, helpContent }: FinanceKpiCardProps) {
  ...
  if (isLoading) {
    return (
      <div className="min-h-[110px] rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] p-5">
  ...
  return (
    <div className="min-h-[110px] rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] p-5 transition-all duration-150 hover:border-[var(--border-strong)]">
      ...
      <p className={cn("mt-1 text-2xl font-semibold tracking-tight", styles.valueColor)}>{value}</p>
      {delta ? (
